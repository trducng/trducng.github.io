...
What if we just add random noise to the embeddings instead of using gradients?
...
**Result:** Only **1 iteration** needed. The gradient simply updated the "John" token embedding directly.
...
- Interestingly, tokens not in the sentence (Alice, James, Sarah) also received small gradients

## Key Findings

1. **Asymmetric robustness**: Breaking the correct answer is easy; steering to a specific target is hard.

2. **Random noise ≠ adversarial**: Models are robust to random perturbations but vulnerable to gradient-based attacks.

3. **Embedding changes required are large**: Flipping predictions via the last token alone requires reducing cosine similarity to ~0.35.

4. **Gradient leaks to related tokens**: Even tokens not in the input receive gradients—names like "Alice", "James", "Sarah" got small updates when attacking "Mary".

5. **Context tokens matter**: "gave" received significant gradients, suggesting the model uses it heavily for the indirect object prediction.

## Implications

- **Adversarial attacks on LLMs are possible** at the embedding level, but require substantial modifications.
- **Defense via embedding constraints** could be effective—monitoring for large embedding changes might detect attacks.
- **The robustness to random noise** suggests LLMs may be naturally defended against certain types of perturbations through their training dynamics.
